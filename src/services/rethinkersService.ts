...
// Service para cadastrar um novo 'rethinker'
const registerRethinker = async (rethinkerData: any) => {
  try {
    const existingRethinker = await rethinkersRepository.getRethinkerByEmail(rethinkerData.email);
    if (existingRethinker) {
      throw new Error('Já existe um rethinker com este email.');
    }
    const newRethinkerId = await rethinkersRepository.insertRethinker(rethinkerData);
